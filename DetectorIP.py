...
import sys, os
from core.DetectorIPLib import DetectorIPLib
...
def main():
    ...
    if(args.uagent and args.ulist):
        logger.PrintError("You can either specify a user-agent string or let DetectorIP pick random user-agent strings for you from a file.", args.nolog)
        sys.exit(6)
        
    #specify proxy or random
    if(args.proxy and args.xlist):
        logger.PrintError("You can either specify a proxy or let DetectorIP pick random proxy connections for you from a file.", args.nolog)
        sys.exit(7)
        
        
    #init lib
    ipGeoLocRequest = DetectorIPLib(args.target, logger, args.noprint)
